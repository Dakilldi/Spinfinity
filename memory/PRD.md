...
- ✅ **TV Show video — final phrasing** (Feb 2026) : kept "🎉 LE GAGNANT EST" header, removed the "Félicitations / WINS" middle line entirely (TV Show prizeLabel set to ''), prize pill is auto-lifted up to fill the gap when no label.
- ✅ **SEO boost for AdSense** (Feb 2026) : `robots.txt`, `sitemap.xml` (8 URLs with hreflang FR/EN/x-default), JSON-LD WebApplication structured data on landing, meta keywords + author + robots:max-image-preview:large.
- ✅ **/about/ real page** (Feb 2026) : hero "Le tirage au sort, pensé autrement" + 4 story cards (Le pitch, La promesse, Le détail qui change tout, L'engagement) with cyan/gold/pink/green accents + 4 value cards (privacy, craft, fast, free) with SVG icons + "Qui est derrière Spinfinity ?" section + dual CTA.
- ✅ **/contact/ real page** (Feb 2026) : hero "Une question ? Écris-nous" with cyan-accent eyebrow + email pill `hello@spinfinity.app` (mailto link) + 4 reason cards (bug, feature, partnership, GDPR) with custom SVG icons + tip note linking to /help/ and /legal/ + dual CTA.
